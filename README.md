# Java Programming Exercises ☕🚀

A structured collection of **Java programming exercises and practical problems** created to improve coding skills, logical thinking, and problem-solving ability.

This repository is continuously updated with new programs and solutions as I practice different Java concepts and solve programming exercises.

---

## 🎯 Objectives

* Build a strong foundation in Java
* Improve programming logic
* Develop problem-solving skills
* Practice practical coding problems
* Strengthen Java fundamentals
* Maintain consistent coding practice
* Prepare for placement coding rounds and technical interviews

---

## 🛠️ Technology

**Java**

---

## 📁 Repository Structure

```text
Java-Programming-Exercises/
│
├── README.md
│
├── Beverage/
│   ├── Subproblem1.java
│   ├── Subproblem2.java
│   ├── Subproblem3.java
│   └── Subproblem4.java
│
├── Boat Ticket/
│   ├── Subproblem1.java
│   ├── Subproblem2.java
│   ├── Subproblem3.java
│   └── Subproblem4.java
│
├── Cake/
│   ├── Subproblem1.java
│   ├── Subproblem2.java
│   ├── Subproblem3.java
│   └── Subproblem4.java
│
├── Cattle Farm/
│   ├── Subproblem1.java
│   ├── Subproblem2.java
│   ├── Subproblem3.java
│   └── Subproblem4.java
│
├── Coffee Shop/
│   ├── Subproblem1.java
│   ├── Subproblem2.java
│   ├── Subproblem3.java
│   └── Subproblem4.java
│
├── Datamart/
│   ├── Subproblem1.java
│   ├── Subproblem2.java
│   ├── Subproblem3.java
│   └── Subproblem4.java
│
├── Farm/
│   ├── Subproblem1.java
│   ├── Subproblem2.java
│   ├── Subproblem3.java
│   └── Subproblem4.java
│
├── Icecream/
│   ├── Subproblem1.java
│   ├── Subproblem2.java
│   ├── Subproblem3.java
│   └── Subproblem4.java
│
├── Nursery/
│   ├── Subproblem1.java
│   ├── Subproblem2.java
│   ├── Subproblem3.java
│   └── Subproblem4.java
│
├── Paris Art/
│   ├── Subproblem1.java
│   ├── Subproblem2.java
│   ├── Subproblem3.java
│   └── Subproblem4.java
└── ...
````

---

## 📚 Exercises

### 🍹 Beverage

Contains four Java programming subproblems based on the Beverage exercise.

**Status:** ✅ Completed

### 🚤 Boat Ticket

Contains four Java programming subproblems based on the Boat Ticket exercise.

**Status:** ✅ Completed

### 🎂 Cake

Contains four Java programming subproblems based on the Cake exercise.

**Status:** ✅ Completed

### 🐄 Cattle Farm

Contains four Java programming subproblems based on the Cattle Farm exercise.

**Status:** ✅ Completed

### ☕ Coffee Shop

Contains four Java programming subproblems based on the Coffee Shop exercise.

**Status:** ✅ Completed

### 🏪 Datamart

Contains four Java programming subproblems based on the Datamart exercise.

**Status:** ✅ Completed

### 🌾 Farm

Contains four Java programming subproblems based on the Farm exercise.

**Status:** ✅ Completed

### 🍦 Icecream

Contains four Java programming subproblems based on the Nursery exercise.

**Status:** ✅ Completed

### 🍦 Nursery

Contains four Java programming subproblems based on the Icecream exercise.

**Status:** ✅ Completed

### Paris Art

Contains four Java programming subproblems based on the Paris art exercise.

**Status:** ✅ Completed

---

## 🧠 What This Repository Covers

The exercises are designed to strengthen practical Java programming and problem-solving skills, including:

* Variables and Data Types
* Input and Output
* Operators
* Conditional Statements
* Loops
* Methods
* Arrays
* Strings
* Mathematical Operations
* Logical Problem Solving
* Java Fundamentals

---

## 🔄 Practice Process

```text
Understand the Problem
        ↓
Analyze the Requirements
        ↓
Design the Logic
        ↓
Write Java Code
        ↓
Test the Program
        ↓
Fix Errors
        ↓
Add Solution to Repository
```

---

## 📈 Progress

| No. | Exercise       | Subproblems |    Status   |
| --: | -------------- | :---------: | :---------: |
|   1 | Beverage       |      4      | ✅ Completed |
|   2 | Boat Ticket    |      4      | ✅ Completed |
|   3 | Cake           |      4      | ✅ Completed |
|   4 | Cattle Farm    |      4      | ✅ Completed |
|   5 | Coffee Shop    |      4      | ✅ Completed |
|   6 | Datamart       |      4      | ✅ Completed |
|   7 | Farm           |      4      | ✅ Completed |
|   8 | Icecream       |      4      | ✅ Completed |
|   9 | Nursery        |      4      | ✅ Completed |
|  10 | Paris Art      |      4      | ✅ Completed |
|  11 | More Exercises |      —      |  ⏳ Upcoming |

---

## 🚀 Continuous Practice

This repository will be continuously updated with new Java programming exercises and solutions.

The goal is to follow:

**Practice → Problem Solving → Improvement → Consistency**

---

## 👨‍💻 Author

**Sahil Patil**

B.Tech Computer Science & Engineering Student

Focused on **Java Development, Backend Development, Full Stack Development, and Data Structures & Algorithms**.

---

## ⭐ Repository Purpose

This repository serves as a personal collection of Java programming exercises to track learning progress, strengthen programming fundamentals, and improve problem-solving skills for **placements and technical interviews**.

```
